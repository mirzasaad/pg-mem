...
export interface SchemaField
  extends Omit<CreateColumnDef, "dataType" | "kind" | "name"> {
  type: IType | DataType;
  name: string;
  serial?: boolean;
}
...
export interface LibAdapters {
  ...
  createDrizzlePg(queryLatency?: number): { Pool: any; Client: any };
...
export type GlobalEvent =
  | "query"
  | "query-failed"
  | "catastrophic-join-optimization"
  | "schema-change"
  | "create-extension";
...
export class CastError extends QueryError {
  constructor(
    from: string | DataType | IType,
    to: string | DataType | IType,
    inWhat?: string
  ) {
    super(
      `cannot cast type ${typeof from === "string" ? from : from.name} to ${
...
export class TypeNotFound extends QueryError {
  constructor(t: string | number | DataTypeDef) {
    super(
      `type "${typeof t !== "object" ? t : typeDefToStr(t)}" does not exist`
    );
  }
}
